/**
 * \file lib/generic/modulation/DemoOfdmPreambleDetector.h
 * \version 1.0
 *
 * \section COPYRIGHT
 *
...
#ifndef MOD_DEMOOFDMPREAMBLEDETECTOR_H_
#define MOD_DEMOOFDMPREAMBLEDETECTOR_H_
...
#include "utility/PythonPlotter.h"

namespace iris
{

/// Detect OFDM preambles using Schmidl & Cox algorithm.
class DemoOfdmPreambleDetector
  : boost::noncopyable
{
public:
   ...
   */
  DemoOfdmPreambleDetector(int symbolLen = 256,
                           int cyclicPrefixLen = 16,
                           float threshold = 0.827,
                           PythonPlotter* plot = NULL)
    :sLen_(symbolLen)
  ...
  void reset()
  {reset(plotter_,sLen_,cpLen_,thresh_/cpLen_);}

  /// Reset the detector.
  void reset(PythonPlotter* plot, int symbolLen, int cyclicPrefixLen, float threshold)
  {
    plotter_ = plot;
    sLen_ = symbolLen;
    cpLen_ = cyclicPrefixLen;
  ...
  static std::string getName(){ return "DemoOfdmPreambleDetector"; }
...
private:
  ...
  PythonPlotter* plotter_;

};

template <class Iterator>
Iterator DemoOfdmPreambleDetector::search(Iterator inBegin,
                                      Iterator inEnd,
                                      ...
                                      float &snr)
{
  ...
  for(; inBegin != inEnd; ++inBegin)
  {
    ...
    if(vMovingAve_ > thresh_ && vMovingAve_ < lastVma_)
    {
      detected = true;
      if(plotter_ != NULL)
        plotter_->plot(vDebugBuffer_.begin(), vDebugBuffer_.end(), "Correlator", 1);
...
#endif // MOD_DEMOOFDMPREAMBLEDETECTOR_H_
