/**
 * \file components/gpp/phy/DemoOfdmDemodulator/DemoOfdmDemodulatorComponent.cpp
 * \version 1.0
 *
 * \section COPYRIGHT
 *
 ...
 * \section DESCRIPTION
 *
 * Implementation of the DemoOfdmDemodulator component.
 */

#include "DemoOfdmDemodulatorComponent.h"
...
IRIS_COMPONENT_EXPORTS(PhyComponent, DemoOfdmDemodulatorComponent);

DemoOfdmDemodulatorComponent::DemoOfdmDemodulatorComponent(std::string name)
  : PhyComponent(name,                                          // component name
                "demoofdmdemodulator",                          // component type
                "A demonstration OFDM demodulation component",  // description
                "Paul Sutton",                                  // author
                "1.0")                                          // version
    ,numHeaderBytes_(7)
...
void DemoOfdmDemodulatorComponent::initialize()
{
  setup();
}

void DemoOfdmDemodulatorComponent::process()
{
...
void DemoOfdmDemodulatorComponent::parameterHasChanged(std::string name)
{
  ...
  if(name == "threshold")
    detector_.reset(&plotter_,numBins_,cyclicPrefixLength_x,threshold_x);
}

void DemoOfdmDemodulatorComponent::setup()
{
...
DemoOfdmDemodulatorComponent::CplxVecIt
DemoOfdmDemodulatorComponent::searchInput(CplxVecIt begin, CplxVecIt end)
{
  float snr(0);
...
DemoOfdmDemodulatorComponent::CplxVecIt
DemoOfdmDemodulatorComponent::processFrame(CplxVecIt begin, CplxVecIt end)
{
  for(; begin != end; begin++)
  {
    if(!haveHeader_)
...
void DemoOfdmDemodulatorComponent::extractPreamble()
{
...
void DemoOfdmDemodulatorComponent::extractHeader()
{
...
void DemoOfdmDemodulatorComponent::demodFrame()
{
...
void DemoOfdmDemodulatorComponent::demodSymbol(CplxVecIt inBegin, CplxVecIt inEnd,
...
void DemoOfdmDemodulatorComponent::generateFractionalOffsetCorrector(float offset)
{
  float relFreq = -offset/numBins_;
  ToneGenerator::generate(corrector_.begin(), corrector_.end(), relFreq);
  plotter_.plot(corrector_.begin(), corrector_.end(), "Corrector", 2);
}

void DemoOfdmDemodulatorComponent::correctFractionalOffset(CplxVecIt begin,
                                                       CplxVecIt end)
{
  transform(begin, end, corrector_.begin(), begin, _1*_2);
}

int DemoOfdmDemodulatorComponent::findIntegerOffset(CplxVecIt begin, CplxVecIt end)
{
...
void DemoOfdmDemodulatorComponent::generateEqualizer(CplxVecIt begin, CplxVecIt end)
{
  ...
  plotter_.plot(equalizer_.begin(), equalizer_.end(), "Equalizer", 3);
}

void DemoOfdmDemodulatorComponent::equalizeSymbol(CplxVecIt begin, CplxVecIt end)
{
