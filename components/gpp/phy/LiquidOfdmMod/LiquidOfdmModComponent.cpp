  ...
  ,frameGenerator_(0)
{
    ...
    registerParameter("debug", "whether to write debug information to file", "false", true, debug_x);

    registerParameter("subcarriers", "Number of subcarriers", "64", true, noSubcarriers_x);
    registerParameter("prefixlength", "Cyclic prefix length", "16", true, cyclicPrefixLength_x);
    registerParameter("taperlength", "Taper length", "4", true, taperLength_x);
    registerParameter("modulation", "Modulation scheme", "qpsk", true, modulationScheme_x);

    registerParameter("fec0", "Forward Error Correction for header", "none", true, fecZero_x);
    registerParameter("fec1", "Forward Error Correction for data", "h128", true, fecOne_x);
    registerParameter("crc", "Cyclic redundancy check", "crc32", true, crcScheme_x);

    registerParameter("numsymbolss0", "number of S0 training symbols", "3", true, numSymbolsS0_x);

    registerParameter("frameheader", "Header content, eight characters long", "default", true, frameHeader_x);
}
...
void LiquidOfdmModComponent::initialize()
{
    ...
    ofdmflexframegenprops_init_default(&fgProps_);   // initialize to default
    ...
    fgProps_.check             = check;
    fgProps_.fec0              = fec0;
    fgProps_.fec1              = fec1;
    fgProps_.mod_scheme        = ms;

    // create frame generator object
    try {
        frameGenerator_ = ofdmflexframegen_create(noSubcarriers_x, cyclicPrefixLength_x, taperLength_x, p, &fgProps_);
