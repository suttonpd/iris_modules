                               ...
                               unsigned char * _p)
{
    ...
    delay  = 13;       // prototype filter delay
    float As        = 60.0f;    // filter stop-band attenuation
    channelizer = firpfbch_crcf_create_kaiser(LIQUID_SYNTHESIZER, num_channels, delay, As);

    // channelizer input/output arrays
    X = (std::complex<float>*) malloc( num_channels * sizeof(std::complex<float>) );
    x = (std::complex<float>*) malloc( num_channels * sizeof(std::complex<float>) );
...
unsigned int multichanneltx::GetFrameSize()
{
  unsigned int maxSyms = 0;
  unsigned int i;
  for(i=0;i<num_channels;i++){
    unsigned int syms = ofdmflexframegen_getframelen(framegen[i]);
    if(syms > maxSyms) maxSyms = syms;
  }
  maxSyms++; //Add frame guard symbol
  return (maxSyms*(M+cp_len)+delay)*num_channels;
}
