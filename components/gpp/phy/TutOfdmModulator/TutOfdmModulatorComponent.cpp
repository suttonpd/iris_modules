/**
 * \file components/gpp/phy/TutOfdmModulator/TutOfdmModulatorComponent.cpp
 * \version 1.0
 *
 * \section COPYRIGHT
 *
 ...
 * \section DESCRIPTION
 *
 * Implementation of the TutOfdmModulator component.
 */

#include "TutOfdmModulatorComponent.h"
...
IRIS_COMPONENT_EXPORTS(PhyComponent, TutOfdmModulatorComponent);

TutOfdmModulatorComponent::TutOfdmModulatorComponent(std::string name)
    ...
    ,framePlot_(NULL)
{
  ...
  registerParameter(
    "plotpreamble", "Whether to plot the generated preamble?",
    "false", true, plotPreamble_x);

  registerParameter(
    "plotbins", "Whether to plot the generated preamble bins?",
    "false", true, plotBins_x);

  registerParameter(
    "plotframe", "Whether to plot the generated frame?",
    "false", true, plotFrame_x);
...
TutOfdmModulatorComponent::~TutOfdmModulatorComponent()
{
  destroy();
  if(preamblePlot_)
    delete preamblePlot_;
  if(binsPlot_)
    delete binsPlot_;
  if(framePlot_)
    delete framePlot_;
...
void TutOfdmModulatorComponent::initialize()
{
  setup();
}

void TutOfdmModulatorComponent::process()
{
...
void TutOfdmModulatorComponent::parameterHasChanged(std::string name)
{
...
void TutOfdmModulatorComponent::setup()
{
  ...
  {
    if(!preamblePlot_)
      preamblePlot_ = new Complexplot("Generated Preamble (Time)");
    preamblePlot_->setNewData(preamble_.begin(), preamble_.end());
  }
...
void TutOfdmModulatorComponent::destroy()
{
  if(fftBins_ != NULL)
    fftwf_free(fftBins_);
  if(fft_ != NULL)
 ...
 */
void TutOfdmModulatorComponent::createHeader(ByteVecIt begin, ByteVecIt end)
{
  //Add the CRC
 ...
 */
void TutOfdmModulatorComponent::createFrame(ByteVecIt begin, ByteVecIt end)
{
  ...
  {
    if(!framePlot_)
      framePlot_ = new Complexplot("Generated Frame (Time)");
    framePlot_->setNewData(out->data.begin(), out->data.end());
  }
 ...
 */
void TutOfdmModulatorComponent::createSymbol(CplxVecIt inBegin, CplxVecIt inEnd,
                                          CplxVecIt outBegin, CplxVecIt outEnd)
{
  ...
  {
    if(!binsPlot_)
      binsPlot_ = new Complexplot("Generated Symbol (Freq)");
    binsPlot_->setNewData(&fftBins_[0], &fftBins_[numBins_]);
  }
...
TutOfdmModulatorComponent::CplxVecIt
TutOfdmModulatorComponent::copyWithCp(CplxVecIt inBegin, CplxVecIt inEnd,
                                   CplxVecIt outBegin, CplxVecIt outEnd)
{
